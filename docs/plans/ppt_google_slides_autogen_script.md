...
> **슬라이드 문구 보강 작업의 진행 기록(이어하기)**: 제목·불릿·노트를 바꿀 때는 `docs/plans/ppt_aux_instructor_build_guide.md` 상단 **「문서 보강 작업 기록 (이어하기용)」** 절에 변경 요약·백로그·다음 작업을 남긴다. 본 파일(`DECK`)만 수정하고 마스터 로그를 비우지 않는다.

> **수동 “실제 작성 덱” 번호·트랙 정본**: **슬라이드 5 = Swagger·`M05`**, **슬라이드 6 = 데모 동선**(브라우저 시연·`lecture_talk` `A01` 도입 등과 연계), **슬라이드 7 = 2-Tier·기술 스택·`A02`**, 탭 진입 브리지 **`A03`**, **슬라이드 11 = 마무리·프로젝트 보강 방향·`R01`·WRAPUP**(도메인 규칙·스키마 재정립 → RAG/Agent/(선택) GraphRAG 확장 준비 — **(구) KPI·3축 중심 카피 대신** `ppt_aux` 「실제 작성 덱 — 슬라이드 11」권장안) — 전부 `ppt_aux_instructor_build_guide.md`·`lecture_talk_track_2026_03_28.md`와 동일하다. 아래 `DECK` 배열의 **순서·내부 `id`(예: `S06`, `R01`)는 자동 생성 덱 내부 순서**일 뿐이며, **강의 슬라이드 번호·`M0x`/`A0x`/`R01` 매핑은 `ppt_aux` 수동 덱 정본을 우선**한다.
