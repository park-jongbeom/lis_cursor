...
## 10. 보강 계획·실행 메모 (2026-03-27)

- 목표: 데모에서 "업로드는 되지만 결과가 빈 값"처럼 보이는 상황을 줄이기 위해 샘플의 시계열 길이·고객 분포·지역/검사 조합을 확장.
- 반영:
  - `scm_sample.csv`: 30행 -> 72행 (3품목 x 24주), 완만한 추세+주기 변동
  - `crm_sample.csv`: 24행 -> 45행 (15고객 x 3주문), 고위험/중간/활성 고객 혼합
  - `bi_sample.csv`: 36행 -> 90행 (5지역 x 2검사 x 9기간), 히트맵·YoY·Top tests 동시 시연 강화
- 추가 보강(이미지 기반): `ref_files/idr_screenshots` 화면 컬럼을 직접 확인해 `lis_orders_sample.csv`를 신규 추가.

### 10.1 이미지 기반 컬럼 추출 요약

| 화면 파일 | 확인한 핵심 컬럼/지표 |
|-----------|-------------------------|
| `검사의뢰조회.png` | 의뢰일자, 의뢰번호, 고객코드/거래처, 검사코드/검사명, 검체코드/검체명, 등록일시 |
| `고객매출현황.png` | 고객별 월별 건수/매출, Total(Sum), 월 추이 |
| `국내지역별매출현황.png` | 월/지역별 매출, 지역 분포, 검사항목/종별코드 비중 |
| `검사항목별매출현황.png` | 검사대분류/중분류/검사항목(코드)별 매출, 월별 매출 테이블 |
| `고객등록현황.png` | 최근 1년 거래 고객수, 연도별/월별 거래·등록 추이 |

### 10.2 샘플 보강 반영

- 신규: `lis_orders_sample.csv` (화면형 원본 스키마)
  - 필드: `request_date`, `request_no`, `customer_code`, `customer_name`, `region`, `test_code`, `test_name`, `specimen_code`, `specimen_name`, `department_code`, `sales_amount`
  - 목적: 강의에서 "실제 업무 화면 형식 -> 분석용 컬럼 변환" 과정을 설명할 때 사용
- 기존 3종(`scm`/`crm`/`bi`)은 데모 버튼 즉시 실행용 정제 데이터로 유지
